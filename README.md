...
**Notification email:** every submission also emails `NOTIFY_EMAILS` (top of the script) — currently Faus and Joan, plus a placeholder slot for Ryan pending his real address. The Form asks gender, nationality, year, major/school, and "Who connected you?" directly, so the email reports those as typed by the respondent. The connector shows up as **two separate lines, never merged**: "Who they said connected them (form)" is exactly what the respondent typed (or blank — it's the one optional Form question), and "Point person on file (tracker)" is who's *currently* responsible for following up — **deliberately never guessed from `contacts.connector`**. Point person only ever shows a real name once a member explicitly assigns one via the tracker's Edit dialog ("Connector," "Suggested point person," or "Other" — all explicit choices); until then it reads "Not yet assigned," and the form-vs-tracker cross-check skips comparing against that placeholder rather than treating it as a mismatch. When ownership *is* explicitly assigned to someone other than the connector, the line adds "(reassigned from X)". `contacts.connector` ("Outreached by" on the website) is purely day-outreach/website info — a member met this person in person and typed themselves in via Quick Add or the tracker's Add Contact dialog. This script deliberately never writes it: a cold Form sign-up has nobody who outreached to them, so `createContact()` leaves it unset, and their Form answer goes only to `last_form_connector` instead. None of the three creation paths touch point person — that's a separate, always-manual step. Showing the form's answer and the tracker's current point person side by side lets the team actually confirm they agree, rather than silently trusting one over the other — if they *conflict*, the cross-check section flags that explicitly. This answer also gets saved to `contacts.last_form_connector` on the matched/created contact, so it's not just visible in this one email — the tracker's website (all tabs) shows an "Outreached by" field (`contacts.connector`) and a separate "Who connected you?" field (the Form's own answer, `contacts.last_form_connector`), each flagged against the point person when they disagree with it. A contact met in person and later self-registering for an event via the Form still shows up under the Event Signups tab too — that tab now includes anyone with an actual event registration, not just contacts whose original `source` was the Form. The email also always shows `contacts.source` for a matched contact (added manually by a member vs. self-submitted via `signup.html`/the Form). Multiple matches or an unrecognized event still send an email, just flagging what needs manual attention instead of a clean cross-check. Sent via `MailApp`, so no extra credentials beyond the script's own Google account — well within its free daily quota for this volume.
