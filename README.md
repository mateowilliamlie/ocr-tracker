...
Submitting the Form marks someone **registered** in `event_attendance.registered` — this is the "signed up" stage. It's the same column `attendance.html`'s matrix reads and writes, so a member can also check someone off manually there (e.g. someone who shows up without ever filling out the Form). Alongside the registration write, the script also inserts an `event_interest` row (interest is even earlier than registered — someone might express interest without ever formally signing up) — that's the same table the tracker's own per-event interest checkboxes read from, so a Gform sign-up shows up as interested there too, not just registered. There's no separate "confirmed showed up at the actual event" stage yet — that's future work, not tracked today.

If `event_attendance` still has the old `attended` column instead of `registered` (e.g. from before this distinction existed), rename it in place — this preserves all existing data with zero duplication:
```sql
