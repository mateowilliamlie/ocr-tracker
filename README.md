...
If their event answer doesn't match a configured event, the contact is still matched/created as above, just without marking attendance for it (and the response row notes that so it doesn't look silently ignored). The Form's event question answer includes the full description text ("BBQ Night: Wed, Aug 26, 7PM @HKUST BBQ Pit - Grill, chat..."), not just the event name, so `EVENT_ID_BY_LABEL` is matched by "answer starts with this label" rather than exact equality — keep its keys as just the short event name.

**Notification email:** every submission also emails `NOTIFY_EMAILS` (top of the script) — currently Faus and Joan, plus a placeholder slot for Ryan pending his real address. The Form asks gender, nationality, year, major/school, and "Who connected you?" directly, so the email reports those as typed by the respondent. "Who connected you?" is the one optional question on the Form (everything else is required), so if it's left blank and there's an existing tracker match, the connector already on file for that contact is used instead — clearly labeled "from tracker" so it's never confused with what the respondent actually typed. Quick Add and the tracker's own Add Contact dialog both write to the same `contacts.connector` column, so it doesn't matter which one a member used. If the respondent's answer instead *conflicts* with what's already on file, the email flags that as a mismatch to confirm rather than silently picking one. The email also always shows `contacts.source` for a matched contact (added manually by a member vs. self-submitted via `signup.html`/the Form). Multiple matches or an unrecognized event still send an email, just flagging what needs manual attention instead of a clean cross-check. Sent via `MailApp`, so no extra credentials beyond the script's own Google account — well within its free daily quota for this volume.

### Setup

1. If `contacts.gender` doesn't exist yet, run `alter table contacts add column gender text;` once in Supabase's SQL Editor — the auto-create path below needs it.
...
4. Fill in the `CONFIG` block at the top of the script:
   ...
   - `EVENT_ID_BY_LABEL` — each event's `id` (Table Editor → `events`, scoped to that season), keyed by the event's short name (e.g. `"BBQ Night"`). Keep this in sync if event names change.
