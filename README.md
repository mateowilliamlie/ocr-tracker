...
**Notification email:** every submission also emails `NOTIFY_EMAILS` (top of the script) — currently Faus and Joan, plus a placeholder slot for Ryan pending his real address. The Form asks gender, nationality, year, major/school, and "Who connected you?" directly, so the email reports those as typed by the respondent. The connector shows up as **two separate lines, never merged**: "Who they said connected them (form)" is exactly what the respondent typed (or blank — it's the one optional Form question), and "Point person on file (tracker)" is who's *currently* responsible for following up — `contacts.connector` from the matched tracker record by default, but the tracker's own Edit dialog can reassign that to someone else (suggested connection or a custom name), and this line always reflects whichever is actually operative, not just the original connector. When that reassignment applies, the line adds "(reassigned from X)" so it doesn't read as a fresh mismatch. Quick Add and the tracker's own Add Contact dialog both write to the same `contacts.connector` column initially, so it doesn't matter which one a member used to first add someone. Showing the form's answer and the tracker's current point person side by side lets the team actually confirm they agree, rather than silently trusting one over the other — if they *conflict*, the cross-check section flags that explicitly. The tracker's website (Event Signups and In Person tabs) shows this same distinction — a "Connected by" field and a separate "Point person" field, flagged when they differ. The email also always shows `contacts.source` for a matched contact (added manually by a member vs. self-submitted via `signup.html`/the Form). Multiple matches or an unrecognized event still send an email, just flagging what needs manual attention instead of a clean cross-check. Sent via `MailApp`, so no extra credentials beyond the script's own Google account — well within its free daily quota for this volume.
