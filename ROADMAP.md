...
- [x] **Three-stage event funnel**: interested (`event_interest`) -> registered (`event_attendance.registered`, new column) -> attended (`event_attendance.attended`). This fixed a real conflation bug — `event-signup-sync.gs` previously marked someone `attended: true` the moment they submitted the Form, which is wrong; submitting only means they registered. Renamed `markAttended()` to `markRegistered()` and changed what it writes; the script never sets `attended` itself, and never overwrites an existing `attended: true` when updating a row (partial `PATCH` bodies in PostgREST only touch fields present in them). `attendance.html`'s split-cell matrix and single-event drill-down became three-way (was two: interested/attended); actual attendance confirmation is now a separate manual step from the Form sign-up, done by whoever's checking people in at the event. See "Event Sign-up Sync" in README.md for the migration SQL.
