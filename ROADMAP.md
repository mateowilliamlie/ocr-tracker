...
- [x] `attendance.html` matrix now shows Contact (WhatsApp/Instagram) and Point person (current follow-up owner, not necessarily who originally connected them) as always-visible sticky columns next to Name, so outreach follow-up doesn't require flipping back to the tracker. A "Show details" toggle reveals Gender/Year/Major/Nationality/Connected-by as additional columns for every row at once (not per-row expand, since clicking each contact individually doesn't scale once a season has many people) — off by default so the event checkbox grid stays uncluttered. Contact column also flags non-numeric `contacts.phone` values as `(WeChat)`, since that field can hold either a real number or a WeChat handle.
